...
class MRZHelper {
  static List<String>? getFinalListToParse(List<String> ableToScanTextList) {
    ...
    final expectedLineCount = switch (lineLength) {
      30 => 3,
      36 => 2,
      44 => 2,
      _ => null,
    ...
    if (ableToScanTextList.length < expectedLineCount) return null;

    final linesToUse = ableToScanTextList.length > expectedLineCount
        ? ableToScanTextList.sublist(ableToScanTextList.length - expectedLineCount)
        : ableToScanTextList;
  ...
  static List<List<String>> _filterAvailableLines(List<String> lines) {
    ...
    for (final line in lines) {
      ...
      if (line.contains('<')) {
        ...
        if (!isEmpty) {
          if (length > 36) fallback44 = line;
          else if (length > 30) fallback36 = line;
          else fallback30 = line;
        }
      }
    }
    ...
    if (mrz44Lines.length == 1 && fallback44 != '<') {
      mrz44Lines.insert(0,
          fallback44 + '<' * (44 - fallback44.length));
    }
    ...
    if (mrz36Lines.length == 1 && fallback36 != '<') {
      mrz36Lines.insert(0,
          fallback36 + '<' * (36 - fallback36.length));
    }
    if (mrz36Lines.length >= 2) availableLines.add(mrz36Lines);

    // TD1 (30-char, 3 lines) ← THE KEY FIX
    if (mrz30Lines.length >= 3) {
      // If OCR grabbed extra lines, use the last 3 (MRZ is at bottom of card)
